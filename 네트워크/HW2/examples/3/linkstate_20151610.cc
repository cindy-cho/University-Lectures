...
int main(int argc, char *argv[]){
    ...
    while(fgets(messageLine,sizeof(messageLine),tf[1])!=NULL){
        ...
        // printf("%d, %d, %s",start,end,cMessage);
        // printf("Cost : %d, next : %d\n",networkMap[start][end].cost,networkMap[start][end].next);
    ...
    while(fscanf(tf[2],"%d %d %d",&start,&end,&cost)!=EOF){
        ...
        while(it != graph[start].end()){
            if((*it).end==end) {
                flag = 1;
                // printf("find ! : start, end, cost : %d, %d, %d\n",(*it).start,(*it).end,(*it).cost);
